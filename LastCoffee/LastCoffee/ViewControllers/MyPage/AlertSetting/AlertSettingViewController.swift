...
class AlertSettingViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // 알림 시간, 알람 설정
        let time = TokenManager.shared.getValue(key: KeychainKey.alertTime.rawValue) ?? "16"
        let isAlertOn : Bool = TokenManager.shared.getValue(key: KeychainKey.isOnAlert.rawValue) ?? false
    ...
    @objc private func valueChangeAlertSwitch(_ switchView: UISwitch){
        TokenManager.shared.saveValue(key: KeychainKey.isOnAlert.rawValue, value: switchView.isOn)
